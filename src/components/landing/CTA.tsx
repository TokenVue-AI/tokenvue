      ...
      <div className="relative mx-auto max-w-5xl px-4">
        ...
        >
          ...
          <p className="relative mt-5 text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto">
            Drop in {SITE_NAME} in 5 minutes. Replace your routing, retries, budgets and audit logs with one battle-tested gateway.
          </p>
          <div className="relative mt-8 flex flex-wrap justify-center gap-3">
            <a href={APP_URL} className="group inline-flex items-center gap-2 rounded-xl bg-foreground text-background px-6 py-3 text-sm font-medium hover:bg-foreground/90 transition shadow-elevated">
              Start free
              <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" />
            </a>
            <a href="#" className="inline-flex items-center gap-2 rounded-xl glass px-6 py-3 text-sm font-medium hover:border-primary/40 transition">
              Contact sales
            </a>
          </div>
        </motion.div>
    ...
    <footer className="relative border-t border-hairline mt-20">
      <div className="mx-auto max-w-6xl px-4 py-14 grid grid-cols-2 md:grid-cols-6 gap-8">
        <div className="col-span-2">
          ...
          <div className="mt-4 inline-flex items-center gap-2 text-[11px] font-mono text-emerald-glow">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-glow animate-pulse-glow" /> All systems normal
          </div>
        </div>
        {cols.map((c) => (
          <div key={c.t}>
            <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground">{c.t}</div>
            <ul className="mt-3 space-y-2">
              {c.l.map((i) => (
                <li key={i}><a href="#" className="text-sm text-foreground/80 hover:text-foreground transition-colors">{i}</a></li>
              ))}
            </ul>
          </div>
        ))}
      ...
      <div className="border-t border-hairline">
        <div className="mx-auto max-w-6xl px-4 py-6 flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground">
          <div>© {new Date().getFullYear()} {SITE_NAME}, Inc.</div>
