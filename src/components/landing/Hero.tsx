  ...
  return (
    <section className="relative pt-36 pb-24 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-4 text-center">
        ...
        >
          Secure, route, monitor, and optimize traffic across OpenAI, Claude, Gemini, and every major
          LLM — from a single enterprise gateway built for production scale.
        </motion.p>
        ...
        >
          <a href={APP_URL} className="group inline-flex items-center gap-2 rounded-xl bg-foreground text-background px-5 py-3 text-sm font-medium hover:bg-foreground/90 transition shadow-elevated">
            Start building
            <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" />
          </a>
          <a href="#" className="inline-flex items-center gap-2 rounded-xl glass px-5 py-3 text-sm font-medium hover:border-primary/40 transition">
            Book a demo
          </a>
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-14"
        >
          <p className="text-[11px] font-mono uppercase tracking-[0.2em] text-muted-foreground/70">Compatible with</p>
          <div className="mt-5 flex flex-wrap justify-center items-center gap-x-10 gap-y-4 opacity-80">
            {["OpenAI", "Anthropic", "Google", "Meta", "Mistral", "DeepSeek", "Cohere"].map((b) => (
              <span key={b} className="text-sm font-semibold tracking-tight text-muted-foreground hover:text-foreground transition-colors">
                {b}
              </span>
            ))}
          </div>
    ...
    <div className="relative ring-gradient rounded-2xl glow-cyan">
      <div className="glass rounded-2xl p-3 sm:p-5 shadow-elevated">
        ...
        <div className="flex items-center justify-between px-2 pb-3 border-b border-hairline">
          ...
          <div className="text-[11px] font-mono text-muted-foreground">{SITE_NAME}.in / dashboard / production</div>
          <div className="text-[11px] font-mono text-emerald-glow flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-glow animate-pulse-glow" /> 99.998% uptime
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-3 mt-4">
          ...
          <div className="hidden md:flex col-span-2 flex-col gap-1.5 text-xs text-muted-foreground">
            {["Overview","Gateways","Routes","Guardrails","Analytics","Budgets","Logs","Team"].map((l, i) => (
              <div key={l} className={`px-2.5 py-1.5 rounded-md ${i===0 ? "bg-secondary text-foreground" : ""}`}>{l}</div>
            ))}
          </div>
          ...
          <div className="col-span-12 md:col-span-10 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { l: "Requests / min", v: "184,203", d: "+12.4%", c: "text-emerald-glow", icon: Activity },
              ...
              <div className="glass rounded-xl p-4 lg:col-span-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1.5 text-[10px] font-mono">
                    ...
                    <span className="px-1.5 py-0.5 rounded bg-emerald-glow/15 text-emerald-glow">Gemini</span>
                  </div>
                </div>
                <ChartArea />
              </div>
              <div className="glass rounded-xl p-4">
                ...
                <div className="mt-3 space-y-2">
                  {[
                    ...
                    <div key={r.n}>
                      <div className="flex justify-between text-[11px]"><span>{r.n}</span><span className="font-mono text-muted-foreground">{r.p}%</span></div>
...
function ChartArea() {
  const points = [12, 18, 14, 22, 28, 24, 32, 30, 38, 34, 44, 40, 48, 56, 52, 62, 58, 68, 72, 66, 78, 82, 76, 88];
  const max = Math.max(...points);
  const w = 600, h = 120, step = w / (points.length - 1);
